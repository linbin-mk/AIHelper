...
系统 SHALL 在调用 `createAuthCard` 之前将交互状态存入 `window._pendingInteraction`，以便在 DOM 需要重建时恢复卡片并重新绑定 Promise。
...
- **AND** `window._pendingInteraction` 包含 `type: 'request_auth'`、`toolCallId`、`args`、`resolve`、`reject`
...
#### Scenario: 从 _pendingInteraction 恢复授权卡片
- **WHEN** `renderChatMessages` 检测到消息中包含未完成的 `request_auth` 工具调用
- **AND** `_pendingInteraction` 存在且匹配此工具调用
- **THEN** 调用 `createAuthCard` 并传入 `args` 中的 `action`、`detail`、`riskLevel`
- **AND** 调用 `handleAuthCard` 绑定交互回调到 `_pendingInteraction.resolve` / `_pendingInteraction.reject`
