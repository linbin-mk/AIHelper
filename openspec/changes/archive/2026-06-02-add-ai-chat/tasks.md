...
- [x] 9.1 测试 Tab 切换：监控 Tab ↔ 聊天 Tab 切换正常，状态保持
- [x] 9.2 测试模型配置：保存配置 → 刷新面板 → 配置正确恢复
- [x] 9.3 测试流式聊天功能：发送消息 → AI 逐字流式输出 → 刷新面板 → 完整聊天记录恢复
- [x] 9.4 测试中断生成：发送消息 → AI 流式输出中点击停止 → 已生成内容保留 → 输入框恢复
- [x] 9.5 测试错误场景：错误 Key → 401 提示；无效 URL → 网络错误提示；断开网络 → 友好提示
- [x] 9.6 测试 Agent 工具调用：询问 AI "当前有哪些请求"→ AI 调用 get_captured_requests 工具 → 返回请求数据
- [x] 9.7 测试清空聊天记录：点击清空 → 确认 → 记录被清除
- [x] 9.8 在 Chrome 扩展管理页面加载扩展，验证所有功能正常
