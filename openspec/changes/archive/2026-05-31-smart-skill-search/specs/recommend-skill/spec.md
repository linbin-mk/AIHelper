...
系统 SHALL 注册一个新技能 `recommend-skill`，技能名称为"推荐Skill"，分类为"业务"。该技能通过 MD 文件方式注册（`skills/recommend-skill/skill.{cn,en}.md`），并在 `skills/manifest.json` 中声明。技能不声明专属工具，不注册 `SkillUIDelegate`，完全依赖现有内置工具（`ask_user` 及收藏管理四工具）完成推荐流程。
