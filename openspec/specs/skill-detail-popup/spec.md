...
- **THEN** 弹窗正文展示该技能 `skill.<lang>.md` 文件的 Markdown 渲染内容（包含标题、列表、代码块等格式）
...
弹窗右上角 SHALL 提供"使用"按钮，按钮文字通过 i18n 国际化。点击后执行：切换到聊天 Tab、激活该 Skill、聊天输入框填入 `/skill-id `（Skill id + 空格），光标定位在空格之后。
